...
# Filter ‘DVC STATUS’ for ‘N, P, or blank’ (not D) and delete them
#####

df_1 = df_1.query("WOFF_STATUS != 'D'")
df_1 = df_1.query("HI_STATUS != 'D'")
df_1 = df_1.query("DVC_STATUS == 'D'")

# Condition 1: If missing from inventory due to missing DR ‘Done’
condition_1 = df_1['DR_STATUS'] != 'D'

# Condition 2: If no volume has been registered in inventory
condition_2 = (
    ((df_1['BLAL_CRUISE_M3_VOL'].isnull()) | (df_1['BLAL_CRUISE_M3_VOL'] == 0))
)

# Condition 3: If no spatial data has been registered for inventory
condition_3 = df_1['SPATIAL_LINKED'] == 'NO'

# Condition 4: If missing a mandatory UBI  
condition_4 = df_1['UBI'].isnull()

# Condition 5: If planned for deletion
condition_5 = df_1['DEL_STATUS'] == 'P'

# Make a list for all the conditions and their corresponding column names
conditions = [
    (condition_1, "No DR 'Done' prior to DVC 'Done'"),
    (condition_2, "Block and R/W cruise volume is blank or 0"),
    (condition_3, "No spatial data linked for block"),
    (condition_4, "No LRM UBI assigned for DVC 'Done'"),
    (condition_5, "DVC 'Done' can NEVER be deleted")
]
...
# output_name_pattern = '../Reports/Ready_to_Sell_(RTS)_Timber_Inventory_Conformance_Summary_YYYY_MM_DD.xlsx'
...
# Pattern of the output file 
output_name_pattern = f'{output_folder}/Ready_to_Sell_(RTS)_Timber_Inventory_Conformance_Summary_YYYY_MM_DD.xlsx'
