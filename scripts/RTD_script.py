...
# Filter ‘DVS STATUS’ for ‘D’ and delete them
# Filter ‘DVC STATUS’ for ‘D’ and delete them
# Filter ‘DR_STATUS’ ‘N, P, or blank’ (not D) and delete them
#####
...
# Condition 1: If no volume has been registered in inventory
condition_1 = (
    ((df_1['BLAL_CRUISE_M3_VOL'].isnull()) | (df_1['BLAL_CRUISE_M3_VOL'] == 0))
)

# Condition 2: If no spatial data has been registered for inventory
condition_2 = (
    (df_1['SPATIAL_LINKED'] == 'NO') & 
    ((df_1['DEL_STATUS'].isnull()) | (df_1['DEL_STATUS'] == 'N'))
)

# Condition 3: If missing from inventory due to missing or future DR 'Done' date
condition_3 = (df_1['DR_Status_Date'].isnull()) | (df_1['DR_Status_Date'] > date_value)

# Condition 4: If Development Started for DIP Timber Inventory and >$1000 spent on layoutand >$1000 spent on layout
condition_4 = (df_1['Layout_Amount'] > 1000)

# Condition 5: If Development for DIP Timber Inventory has started Apr 1 2020 later and >$0 spent on layout
condition_5 = (
...
conditions = [
    (condition_1, "Block and RW cruise volume is blank or 0"),
    (condition_2, "No spatial data linked for blocks without Deletion Approval 'Planned' activity"),
    (condition_3, "DR 'Done' date is blank or in the future"),
    (condition_4, "Development Started (>$1000 spent on layout; helicopter/water taxi included) without Development Started 'Done'"),
    (condition_5, "Development Started (>$0 spent on layout; helicopter/water taxi included) April 1, 2020 or later without Development Started 'Done'")
]
...
# The final dataframe that will be used to generate t# The final dataframe that will be used to generate the report
selected_df = selected_df[columns_kept]

# # Pattern of the output file 
# output_name_pattern = '../Reports/Ready_to_Develop_Timber_Inventory (RTDTI)_Conformance_Summary_YYYY_MM_DD.xlsx'
...
# Pattern of the output file 
output_name_pattern = f'{output_folder}/Ready_to_Develop_Timber_Inventory (RTDTI)_Conformance_Summary_YYYY_MM_DD.xlsx'
...
with pd.ExcelWriter(output_path) as writer:
    ...
    for sheet_name in selected_df['BUSINESS_AREA_CODE'].unique().tolist() + ['Province']: # Iterate through each regional + province sheet
        ...
        for row_idx, (_, row) in enumerate(region_df.iterrows(), start=1): # Iterate through each row of region_df and apply condition highlighting format
            ...
            if (row['BLAL_CRUISE_M3_VOL'] == '') | (row['BLAL_CRUISE_M3_VOL'] == 0):
                worksheet.write(row_idx, column_to_index['DR_STATUS'], row['DR_STATUS'], yellow_format)
            ...
            if (
                ((row['DEL_STATUS'] == 'N') or (row['DEL_STATUS'] == '')) and
            ...
            if ((row['DR_Status_Date'] == '') or (row['DR_Status_Date'] > date_value)):
                worksheet.write(row_idx, column_to_index['DR_STATUS'], row['DR_STATUS'], yellow_format)
                worksheet.write(row_idx, column_to_index['DR_Status_Date'], row['DR_Status_Date'], red_format)    

            if (row['Layout_Amount'] != '') and (row['Layout_Amount'] > 1000):
            ...
            if (
                ((row['First_Layout_Date'] != '') and (row['First_Layout_Date'] >= datetime(2020, 4, 1))) and
                ((row['Layout_Amount'] != '') and (row['Layout_Amount'] > 0))
            ):
                worksheet.write(row_idx, column_to_index['Layout_Amount'], row['Layout_Amount'], yellow_format)
                worksheet.write(row_idx, column_to_index['First_Layout_Date'], row['First_Layout_Date'], yellow_format)
                worksheet.write(row_idx, column_to_index['DVS_STATUS'], row['DVS_STATUS'], red_format) 
