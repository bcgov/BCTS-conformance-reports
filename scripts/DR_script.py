...
# Condition 1: If a licence was issued this fiscal with no Annual DR claimed
condition_1 = (df_1['HI_STATUS'] == 'D') & (df_1['DR_STATUS'] != 'D')

# Condition 2: If a block was toggled to ‘Development Completed’ this fiscal with no Annual DR claimed
condition_2 = (
    (df_1['DVC_STATUS'] == 'D') & 
...
# Condition 3: If Annual DR was never claimed because of a missing or future date
condition_3 = (
    (df_1['DR_STATUS'] == 'D') &
    ((df_1['DR_Status_Date'].isnull()) | (df_1['DR_Status_Date'] > date_value)) #what is the 9999-99-99?
)

# Condition 4: If Annual DR was claimed without a credit for volume 
condition_4 = (
    (df_1['DR_STATUS'] == 'D') &
    (df_1['DR_FISCAL'] == current_fiscal) &
    ((df_1['BLAL_CRUISE_M3_VOL'].isnull()) | (df_1['BLAL_CRUISE_M3_VOL'] == 0))
)

# Condition 5: If Annual DR was claimed without mandatory spatial data
condition_5 = (
    (df_1['DR_STATUS'] == 'D') &
    (df_1['DR_FISCAL'] == current_fiscal) &
...
# Condition 6: If Annual DR was claimed when or after the block was written off 
condition_6 = (
    (df_1['DR_STATUS'] == 'D') &
    (df_1['DR_FISCAL'] == current_fiscal) &
    (df_1['WOFF_STATUS'] == 'D') &
    (df_1['DR_Status_Date'] >= df_1['WOFF_Status_Date'])
)

# Make a list for all the conditions and their corresponding column names
conditions = [
    (condition_1, "Licence issued this fiscal without DR 'Done'"),
    (condition_2, "DVC 'Done' this fiscal with no DR 'Done' claimed"),
    ...
    (condition_6, "Annual DR declared at or after time of write-off")
]
...
# output_name_pattern = f'{output_folder}/Annual_Development_Ready_(DR)_Conformance_Summary_YYYY_MM_DD.xlsx'
...
with pd.ExcelWriter(output_path) as writer:
    ...
    for sheet_name in selected_df['BUSINESS_AREA_CODE'].unique().tolist() + ['Province']: # Iterate through each regional + province sheet
        ...
        for row_idx, (_, row) in enumerate(region_df.iterrows(), start=1): # Iterate through each row of region_df and apply condition highlighting format
            ...
            if (row['HI_STATUS'] == 'D') and (row['DR_STATUS'] != 'D'):
            ...
            if (row['DVC_STATUS'] == 'D') and (row['DVC_FISCAL'] == current_fiscal and row['DR_STATUS'] != 'D'):
            ...
            if (row['DR_STATUS'] == 'D') and ((row['DR_Status_Date'] == '') or (row['DR_Status_Date'] > date_value)):
            ...
            if (
                (row['DR_STATUS'] == 'D') and
                (row['DR_FISCAL'] == current_fiscal) and
                ((row['BLAL_CRUISE_M3_VOL'] == '') | (row['BLAL_CRUISE_M3_VOL'] == 0))
            ):
                worksheet.write(row_idx, column_to_index['DR_STATUS'], row['DR_STATUS'], yellow_format)
                worksheet.write(row_idx, column_to_index['DR_FISCAL'], row['DR_FISCAL'], yellow_format)
            ...
            if (
                ...
                (row['DR_Status_Date'] >= df_1['WOFF_Status_Date'])
            ):
                worksheet.write(row_idx, column_to_index['DR_STATUS'], row['DR_STATUS'], yellow_format)
                worksheet.write(row_idx, column_to_index['DR_FISCAL'], row['DR_FISCAL'], yellow_format)
                worksheet.write(row_idx, column_to_index['WOFF_STATUS'], row['WOFF_STATUS'], yellow_format)
                worksheet.write(row_idx, column_to_index['WOFF_Status_Date'], row['WOFF_Status_Date'], yellow_format)
                worksheet.write(row_idx, column_to_index['DR_Status_Date'], row['DR_Status_Date'], red_format)
